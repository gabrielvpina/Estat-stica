...
> #O diagrama de dispersão da Figura 11 (a) mostra uma relação curvilínea entre as 
> #O diagrama de dispersão mostra uma relação curvilínea entre as variáveis (Y,X1) e (Y, X2)
> 
> 
...
> 
> ajuste = lm(dados$v.c.c.Yi~DAP1+I(DAP1^2) + ht1+I(ht1^2) + DAP1*ht1)
> summary(ajuste)

Call:
lm(formula = dados$v.c.c.Yi ~ DAP1 + I(DAP1^2) + ht1 + I(ht1^2) + 
    DAP1 * ht1)
...
> abline(h=0)
> plot(DAP1, residuals(ajuste),xlab="DAP-Média",ylab="Resíduos")
> 
> abline(h=0)
